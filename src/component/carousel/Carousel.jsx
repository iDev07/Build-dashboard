...
export const Carousel = () => {
  ...
  const onChange = async (e) => {
    setEdit(e.target.form[1].value)
    setEditRu(e.target.form[2].value)
  ...
  const editHandleOk = () => {
    form
      ...
      .then((values) => {
        ...
        dispatch(
          updateAction("carousel", UPDATE_CAROUSEL, selectedEditID.id, values)
        );
        dispatch(getAction("carousel", GET_CAROUSEL));
      })
      .catch((info) => {
      ...
      render: (text, record) => {
        return (
          <div>
            <img
              className="tableImg"
              src={process.env.REACT_APP_API_URL + record.attachment.url}
              alt={"rasm yo"}
            />
          </div>
        );
